...
describe("prepareReviewWorkspace", () => {
  ...
  it("creates a Codex-ready review workspace", async () => {
    ...
    await expectFile(result.workspaceDir, "README.md", "Open the `proofread` project root as the Codex workspace.");
    await expectFile(result.workspaceDir, "README.md", "reports/pages/");
    await expectFile(result.workspaceDir, "README.md", "proofread prepare <client-name>");
    ...
    await expectFile(result.workspaceDir, "batches/batch-001-prompt.md", "a `diff` block of the change plus a copyable `text` block of the suggested correction");
    ...
    await expectFile(result.workspaceDir, "page-report-template.md", "```diff");
  ...
  it("auto-discovers a default config file in the working directory", async () => {
    ...
    const input = await createPack(2);
    await writeFile(
      path.join(projectRoot, "proofread.config.yml"),
      "excluded_pages:\n  - page-2\n",
      "utf8"
    );
    ...
    try {
      ...
      expect(result.configPath).toBe(path.join(projectRoot, "proofread.config.yml"));
      expect(result.excludedPageCount).toBe(1);
      expect(result.pageReportCount).toBe(1);
    } finally {
...
async function makeTempDir(): Promise<string> {
  const dir = path.join(os.tmpdir(), `proofread-${Date.now()}-${Math.random()}`);
